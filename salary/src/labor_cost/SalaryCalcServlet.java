	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			*/
			...
			//通常労働と残業労働の境目の8時間(480分)を定義
			final int SecOfRegular = 480;
			 ...
			 * 残業在りの場合と無しの場合)*/
			if (workTime > SecOfRegular) {
				//残業が日中だけの場合
				if (nightTime_work == 0) {
					basicPay = (SecOfRegular / 60) * pay;
					overPay = ((dayTime_work - SecOfRegular) * 100 / 60) * pay / 100 * overtime;
					//残業が日中と深夜時間帯にまたがる場合
				} else if (dayTime_work != 0 && nightTime_work != 0) {
					basicPay = (dayTime_work * pay) / 60;
					overPay = (workTime - SecOfRegular) * 100 / 60 * pay * midnight * overtime / 100;
					nightPay = ((((nightTime_work - (workTime - SecOfRegular)) * 100 / 60) * pay) / 100) * midnight;
					//残業が深夜のみの場合
				} else if (dayTime_work < SecOfRegular) {
					basicPay = (dayTime_work * pay) / 60;
					nightPay = ((SecOfRegular - dayTime_work) * pay / 60) * midnight;
					overPay = ((workTime - SecOfRegular) / 60) * pay * midnight * overtime;
				}
			} else { //残業無し
			...
			if (workTime >= SecOfRegular) {
				OverTimeWork = rps.MinutesFormat((workTime - SecOfRegular));
			}
